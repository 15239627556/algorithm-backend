...
def main() -> None:
    ...
    print(
        f"[INFO][DEDUP] 去重前 cell_type={cfg.cell_type} 数量: {before_target} "
        f"(全部细胞: {before_total})"
    )
    if before_target == 0:
        print(f"[WARN][DEDUP] 未找到 cell_type={cfg.cell_type} 的细胞，跳过去重。")
        return
    ...
    print(f"[INFO][DEDUP] 去重后 cell_type={cfg.cell_type} 数量: {after_target}")
    print(f"[INFO][DEDUP] 去重减少 cell_type={cfg.cell_type} 数量: {removed}")
    print(f"[INFO][DEDUP] 去重后全部细胞数量: {after_total}")
    print(f"[INFO][DEDUP] dedup_cells_across_tiles_per_type 运行时间: {elapsed_s:.6f} 秒")

    output_name = cfg.output_json or f"{json_path.stem}_dedup.json"
    save_path = out_dir / output_name
    out_dir.mkdir(parents=True, exist_ok=True)
    project.save_json(str(save_path))
    print(f"[SUCCESS][DEDUP] 去重后的项目已保存至: {save_path}")
