...
from .data_structure import Rect, SelectionResult, TaskOutput
from .geometry import compute_head_crop
...
class VizConfigRBC:
    # 这里改成你自己的测试 json
    json_path: str = "/home/ubuntu/VScodeProjects/项目json数据/血片评分测试/20260828008/0a76c831e8c74d91b63f29d6d4fcb45a.json"
    # 设置为 .roi.npz 时优先直接读取 RoiDataset；None 时保持 JSON 输入方式。
    # roi_path: str | None = None
    roi_path: str = "/home/ubuntu/VScodeProjects/项目json数据/血片评分测试/20260828008/0a76c831e8c74d91b63f29d6d4fcb45a.roi.npz"
...
def visualize_results(
    ...
    if head_crop_rect is not None and head_crop_rect.w > 0 and head_crop_rect.h > 0:
        head_patch = patches.Rectangle(
            (head_crop_rect.x - 0.5, head_crop_rect.y - 0.5),
            head_crop_rect.w,
            head_crop_rect.h,
            linewidth=2.5,
            edgecolor="lime",
    ...
    ax2.set_title("Figure 2 (RBC): Final Selection, Head Crop & User Constraint")
...
def main() -> None:
    ...
    print(
        f"[INFO][RBC] 从 info 读取: dpi={info.dpi}, heatmap_orientation={info.heatmap_orientation}, "
    ...
    if pipeline.best_res and pipeline.grid:
        head_crop_rect = compute_head_crop(
            pipeline.grid, bm_cfg.heatmap_orientation, bm_cfg
        )
        print(
            f"[INFO][RBC] 头部区域 rect: x={head_crop_rect.x:.1f}, y={head_crop_rect.y:.1f}, "
            f"w={head_crop_rect.w:.1f}, h={head_crop_rect.h:.1f}"
        )
        visualize_results(
