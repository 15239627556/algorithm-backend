...
class X100ImageModels {

public:

    X100ImageModels(int num_workers) {
    ...
    /* 往队列添加一张图片 */
    int enqueue_task(py::array image, int task_type) {
        int task_id;
        ...
        { 
            ...
            task_queue_.push({mat, task_id, ttype});
        }
    ...
    pyTaskResultType get_result(int task_id) {
        ...
        TaskResult result = task_results_[task_id];
...
private:
    ...
    void worker_task_dispatching_loop_(int index) {
        ...
        while (true) {
            ...
            // LOGF("Worker Wrapper Loop %d will upload task ID: %d to slot ID: %d",
            //     index, task.task_id, slot_id);
        }
    }
        ...
        while (true) {
            ...
            for (int i = 0; i < BATCH_SIZE; ++i) {
                if (buffer->blocks[slot_id].task_batch_[i].data_filled) {
                    ...
                    // LOGF("Worker Loop %d received result of task ID: %d",
                    //        index, buffer->blocks[slot_id].task_batch_[i].task_id);
                    buffer->blocks[slot_id].task_batch_[i].data_filled = false;
                    {
                        ...
                        task_results_[buffer->blocks[slot_id].task_batch_[i].task_id] = result;
                    }
                }
            }
    ...
    std::map<int, TaskResult>           task_results_;
