...
public:
    ...
    void add_x40_task(std::vector<cv::Mat>& imgs, WorkerSharedBuffer* buffer, int slot_id) {
        ...
        {
            std::lock_guard<std::mutex> lock(task_mutex);
            image_actual_num = imgs.size();
            for (int i = image_actual_num; i < INPUT_IMAGE_NUM; ++i) {
                imgs.push_back(imgs[0]);
            }
            imageProcessing1(imgs, task->resize_images);
    ...
    {
        {
            ...
            for(int b = 0; b < image_actual_num; b++)
            {
                ...
                {
                    result_.result.imageResultInfos.haveCellCenterPoints[i].c_x = X40HaveCellLocateOutFilter[i].x * 4;
                    result_.result.imageResultInfos.haveCellCenterPoints[i].c_y = X40HaveCellLocateOutFilter[i].y * 4;
                }
                //巨核细胞信息赋值
                ...
                {
                    result_.result.imageResultInfos.bigCellRects[i].x = task->result_x40Big[b].bigCellInfo[i].x;
                    result_.result.imageResultInfos.bigCellRects[i].y = task->result_x40Big[b].bigCellInfo[i].y;
                    result_.result.imageResultInfos.bigCellRects[i].w = task->result_x40Big[b].bigCellInfo[i].width;
                    result_.result.imageResultInfos.bigCellRects[i].h = task->result_x40Big[b].bigCellInfo[i].height;
                }
