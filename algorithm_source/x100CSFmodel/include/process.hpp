...
#include "common.hpp"
#include "x100CSFTask.hpp"

namespace bip = boost::interprocess;


void process_batch(WorkerSharedBuffer* buffer, WorkerDataBlock &block, int slot_id, X100CSFMain *uInfo, int worker_id) {
    ...
    if (inputImages.size() >= 0 && inputImages.size() <= BATCH_SIZE) {
        uInfo->add_csf_task(inputImages, buffer, slot_id);
        for (int i = 0; i < BATCH_SIZE; ++i) {
            TaskDataBlock &task = block.task_batch_[i];
            if (!task.data_filled) continue;
