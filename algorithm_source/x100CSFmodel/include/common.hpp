...
enum TaskTypes {
    CSFHAVECELL   = 0,      /* 脑脊液有核任务 */
};

struct itmCellRcgz_x100CSF{
    int m_type;
    float m_pcnt;
};
//每个细胞位置以及top5
struct itmCellRcgz_x100CSF_image{
    int                                 picIndex; //细胞所在采样图下标
    cv::Rect                            cellRect;
    bool                                cellisempty; // 细胞数据是否是空 true-空， false-有效
    std::vector<itmCellRcgz_x100CSF>    top5list;
};

struct cellMatAndIndex{
    int         cellIndex;
    cv::Mat     cellMat;
};

struct itemX100CSFLocateInfo {
	cv::Mat boxes;
	std::vector<float> scores;
	std::vector<int>   labels;//0-有核(脑脊液只有一类)
};
...
struct ImageResultInfos{
    int                             cellRectsSize;// 单张采样图上巨核(或者有核)细胞实际数量               
    cellRectInfo                    cellRects[CELL_NUM_MAX]; //单张采样图上所有巨核(或者有核)细胞框
    cellClassifyInfos               cellClassifyResult[CELL_NUM_MAX]; //单张采样图上所有巨核(或者有核)细胞分类结果
};
...
pyTaskResultType convert_to_py_task_result(const TaskResult &result){
    ...
    const ssize_t N1 = result.imageResultInfos.cellRectsSize;

    py::dict output;
    output["value"] = result.value;

    std::vector<ssize_t> rect_shape = {N1, 4};
    py::array_t<int> cell_rects(rect_shape);
    auto rects_buf = cell_rects.mutable_unchecked<2>();

    std::vector<ssize_t> cell_type_shape = {N1, 5};
    py::array_t<int> cell_types(cell_type_shape);
    auto types_buf = cell_types.mutable_unchecked<2>();

    std::vector<ssize_t> cell_ratio_shape = {N1, 5};
    py::array_t<float> cell_ratios(cell_ratio_shape);
    auto ratios_buf = cell_ratios.mutable_unchecked<2>();

    const auto& img = result.imageResultInfos;

    for (ssize_t i = 0; i < N1; ++i) {
        rects_buf(i, 0) = img.cellRects[i].x;
        rects_buf(i, 1) = img.cellRects[i].y;
        rects_buf(i, 2) = img.cellRects[i].w;
        rects_buf(i, 3) = img.cellRects[i].h;

        types_buf(i, 0) = img.cellClassifyResult[i].top1;
        types_buf(i, 1) = img.cellClassifyResult[i].top2;
        types_buf(i, 2) = img.cellClassifyResult[i].top3;
        types_buf(i, 3) = img.cellClassifyResult[i].top4;
        types_buf(i, 4) = img.cellClassifyResult[i].top5;

        ratios_buf(i, 0) = img.cellClassifyResult[i].ratio1;
        ratios_buf(i, 1) = img.cellClassifyResult[i].ratio2;
        ratios_buf(i, 2) = img.cellClassifyResult[i].ratio3;
        ratios_buf(i, 3) = img.cellClassifyResult[i].ratio4;
        ratios_buf(i, 4) = img.cellClassifyResult[i].ratio5;
    }

    output["cellRects"] = cell_rects;
    output["cellTypes"] = cell_types;
    output["cellRatios"] = cell_ratios;
...
struct WorkerSharedBuffer {
    ...
    void upload_task(Task &task, int slot_id) {
        ...
        blocks[slot_id].task_batch_[batch_index].task_type = task.task_type;
